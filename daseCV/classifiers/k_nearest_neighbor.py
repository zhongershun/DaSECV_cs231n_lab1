...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i] = ((np.sum((self.X_train - X[i])**2 , axis = 1))**0.5).T 
            pass
    ...
    def compute_distances_no_loops(self, X):
        ...
        # .......
        # [[sum(Xn^2)]]
        dists = np.sqrt((-2)*mul+np.reshape(te,(-1,1))+np.reshape(tr.T,(1,-1)))
        pass
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            mask = list(range(k))
            k_neighbor_arg = np.argsort(dists[i])
            k_neighbor_arg = k_neighbor_arg[mask] #取前n个:            
            
            for x in k_neighbor_arg:    
